...
Кэш должен быть доступен по атрибуту cache и представлять собой словарь, ключом в котором является
кортеж с аргументами, а значением — возвращаемое значение декорируемой функции при вызове с этими аргументами.

Примечание 1. Для однозначного кеширования гарантируется,
...
class CachedFunction:
    ...
    def __call__(self, *args_):
        if args_ not in self.cache:
            self.cache[args_] = self.func(*args_)
        return self.cache[args_]
...
@CachedFunction
def quadratic_polinomial(a_, b_, c_, x_):
    return a_ * x_ ** 2 + b_ * x_ + c_
